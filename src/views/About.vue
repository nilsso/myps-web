<template>
    <div class="w-1/2 mx-auto pt-4 pb-1/3h text-justify">
        <!--<HeaderTag id="what-is-myps">What is MYPS?</HeaderTag>-->
        <h1>What is MYPS?</h1>
...
a = 1
        ...
        registers are <em>automatically allocated</em> when the MYPS is compiled.</p>
...
div r1 r0 r2 # b = (a + b) / c
        ...
        prefix, with the added bonus that defines in MYPS can be constant valued expressions that
...
yield
        ...
        <p><strong>(Note that definitions <em>currently</em> only support expressions of literal
        numbers, but eventually I'll fix this to allow definitions that are expressions of other
...
sb 123 Setting r0
        ...
        within expressions. For a <em>full</em> example, here is a complete two-axis sun tracking
        solar panel program using these grammars:</p>
...
bltal r0 r1 20
        ...
        use of indents to denote a block. <strong>(For now and by default these indents
        <em>must</em> be four spaces but are lexed exactly as they are in Python; I'll change this
        eventually to support configurable indent sizes.)</strong></p>

        <p>We have the <strong>infinite <code>loop</code></strong></p>
        <Example>
            <template>
<pre>
loop:
...
jr -1
        ...
        <p><strong>the <code>tag</code></strong>, the MYPS equivalent to line tags in MIPS</p>
        <Example>
            <template>
<pre>
a = 0
...
j 1
        ...
        <p><strong><code>if-elif-else</code> chains</strong></p>
        <Example>
            <template>
<pre>
a = 0
...
move r0 3
        ...
        <strong><code>while</code> loops</strong>
...
jr -2
        ...
        <p><strong>numerical <code>for</code> loops</strong> (granted with some slightly
        modified grammar)</p>
...
j(CalcC)
        ...
        jumps, <code>a</code> will actually be alive by the time of the <code>c = a * 2</code>
        statement. We might first try to fix this by initializing <code>a</code>.</p>
...
j 2
        ...
        <p>Which compiles, but notice that the register <code>r0</code> which was allocated for
        <code>a</code> has been re-used for the allocation of <code>b</code>, clobbering whatever
        value was in <code>a</code> that needed to be used for the <code>c</code> statement. To
        <em>fix</em> this, we can either prefix the <code>a</code> initialization with the keyword
        <code>fix</code> or by replacing the initialization with a <em>fix declaration</em>.</p>
...
<style lang="postcss">
    .example-group {
        @apply my-4 divide-y divide-gray-600;
        .example {
            @apply my-0;
            .hljs {
                @apply rounded-none;
            ...
            &:first-child {
                pre {
                    &:first-child {
                        .hljs { @apply rounded-tl-lg; }
                    }
                    &:last-child {
                        .hljs { @apply rounded-tr-lg; }
                    }
                }
            }
            &:last-child {
                pre {
                    &:first-child {
                        .hljs { @apply rounded-bl-lg; }
                    }
                    &:last-child {
                        .hljs { @apply rounded-br-lg; }
                    }
                }
            }
        }
        /* .example */
        /* div:first-child div.example { */
            /* @apply my-0 mt-4 mb-4; */
        /* } */
    }
</style>
